...
This is the main class that ties up Mean, Volatility and Distribution classes into the multivariate model
and performs staged maximum likelihood estimations
...
class multModel(object):
    def __init__(self, Data, Mean, Vol, Dist, startingVals=None, params=None):
        # Data dataframe with each column being separate variable
        if Data.shape[1]<2:
            raise ValueError('Too little observations for multivariate model. Use empModel instead')
        self._data = Data
        # Set multivariate mean model
        self._mean = Mean
        self._vol = Vol
